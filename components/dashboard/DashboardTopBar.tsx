...
function getTodayString(): string {
  return new Date().toLocaleDateString('de-DE', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: 'Europe/Berlin' })
}
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        >
          <Bell size={20} />
          {unreadCount > 0 && (
            <span className="absolute -top-0.5 -right-0.5 w-5 h-5 flex items-center justify-center rounded-full text-white font-bold" style={{ fontSize: 10, background: primary }}>
        ...
        <Link href="/dashboard/profil">
          {userImage ? (
            <img src={userImage} alt={userName} className="w-9 h-9 rounded-full object-cover" style={{ outline: `2px solid ${primary}`, outlineOffset: 2 }} />
          ) : (
            <div className="w-9 h-9 rounded-full flex items-center justify-center text-white font-bold text-sm" style={{ background: primary }}>
